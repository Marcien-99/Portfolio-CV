...
- **Raccourci Secret Administration (Triple-clic Footer)** : Implémentation du composant client `AdminSecretLink` autour de la marque `marcien-bn.dev` dans le footer. Un triple-clic rapide (dans un intervalle de 1 seconde) redirige discrètement vers l'interface `/admin`.
