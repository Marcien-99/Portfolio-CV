...
- **Phase 3 — La Vitrine Publique : Téléchargement multi-profils à la carte (Hors PRD)** :
  ...
  - **Intégration globale sur tout le site** :
    ...
    - **Footer (Pied de page)** : Intégré initialement, le bouton de téléchargement a été retiré suite au retour de l'utilisateur, car le bouton du Header reste fixé (sticky) lors du scroll en bas de page, rendant un second bouton sur le footer redondant.
...
- **Fonctionnalités et correctifs implémentés** :
  ...
  - **Correction de la localisation sur le CV PDF** : La génération du CV interrogeait une ancienne clé (`contact_address`) au lieu de la vraie clé enregistrée dans les paramètres globaux (`hero_location`). Le CV généré affiche maintenant correctement la localisation définie dans les réglages au lieu de "Paris, France".
